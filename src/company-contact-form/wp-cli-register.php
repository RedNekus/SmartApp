...
WP_CLI::add_command( 'company-contact verify-hubspot', function( $args, $assoc_args ) {
    return CCF\CLI::verify_hubspot( $args, $assoc_args );
}, [
    'shortdesc' => 'Verify HubSpot connection (test API credentials)',
]);

WP_CLI::add_command( 'company-contact submissions', function( $args, $assoc_args ) {
    return CCF\CLI::submissions( $args, $assoc_args );
}, [
    'shortdesc' => 'List recent form submissions',
]);

WP_CLI::add_command( 'company-contact logs', function( $args, $assoc_args ) {
    return CCF\CLI::logs( $args, $assoc_args );
}, [
    'shortdesc' => 'View logger entries',
]);

WP_CLI::add_command( 'company-contact rotate-logs', function( $args, $assoc_args ) {
