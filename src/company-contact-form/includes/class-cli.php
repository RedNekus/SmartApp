...
class CLI {
    ...
    public static function submissions( $args, $assoc_args ) {
        ...
        \WP_CLI\Utils\format_items( $format, $items, [ 'ID', 'Email', 'Name', 'Date' ] );
    }
    ...
    public static function logs( $args, $assoc_args ) {
        ...
        if ( $result ) {
            $logs = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM $table_name WHERE result = %s ORDER BY timestamp DESC LIMIT %d",
                    $result,
                    $count
                ),
                'ARRAY_A'
        ...
        \WP_CLI\Utils\format_items( 'table', $logs, [ 'id', 'timestamp', 'email', 'result', 'hubspot_id', 'user_ip' ] );
    }
